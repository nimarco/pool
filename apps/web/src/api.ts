...
export interface NeedDraft {
   ...
   *  omission. */
  preferences?: NeedPreferences;
  /** The clarification plan whose questions this form asked, when one was made.
   *
   *  Lineage, not authority — it cannot widen a rule or change what an answer means. It
   *  travels so the coordination event can name the plan submitted with this revision;
   *  reading it back later by searching for the newest plan for the member and product
   *  is how an old declaration came to display a plan made after it. The server checks it
   *  against the member, the product, the Community, and the answers sent beside it. */
