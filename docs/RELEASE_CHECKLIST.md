...
| Public demo URL, reachable with no AWS account | **Verified** | <https://d38kno05ygcarw.cloudfront.net/verify> — CloudFront distribution `EMOLZSGVY7HTN`, `Deployed` and enabled, deployed and driven end to end 2026-09-02. CloudFront is what fixes the blocked `*.lambda-url.*.on.aws` category on Cisco Umbrella and its peers (#0065); publish this hostname, never the raw Function URL. Still worth one check from a genuinely different network (a phone hotspot, not a private window, which shares the machine's resolvers) |
