...
def test_canonical_final_offer_still_issues_unchanged(seeded_ctx):
    """The ordinary path converges on the first pass and is untouched by the contract."""
    pool = _pool_with_host(seeded_ctx)
    ...
    assert len(result.economics["lines"]) == len(_active(seeded_ctx, pool.id)) == 10


def test_pruning_converges_and_reprices_for_the_survivors(seeded_ctx):
    ...
    """
    pool = _pool_with_host(seeded_ctx)
    _reject_on_distribution_day(seeded_ctx, pool, "hh_villanueva")

    result = coord.issue_final_offer(ctx=seeded_ctx, pool_id=pool.id)

    assert result.removed == ["hh_villanueva"]
    survivors = _active(seeded_ctx, pool.id)
...
def test_rejection_on_the_last_permitted_pass_fails_closed(seeded_ctx, monkeypatch):
...
def test_non_convergence_authorises_nobody(seeded_ctx, monkeypatch):
    ...
    assert all(m.final_cost_cents == 0 for m in _active(seeded_ctx, pool.id))
    assert not any(
...
def test_non_convergence_is_recorded_loudly(seeded_ctx, monkeypatch):
    ...
    kinds = [e.kind for e in seeded_ctx.repo.list_activity(WS)]
    assert "final_offer_not_converged" in kinds
    stored = seeded_ctx.repo.get_pool(WS, pool.id)
    assert stored.status == PoolStatus.FAILED
    assert "did not settle" in stored.failure_reason


def test_every_buyer_rejected_still_reports_its_own_reason(seeded_ctx, monkeypatch):
    """Losing the whole pool is a different failure from failing to converge."""
    monkeypatch.setattr(coord, "MAX_PRICING_PASSES", 4)
    pool = _pool_with_host(seeded_ctx)
    for membership in _active(seeded_ctx, pool.id):
        _reject_on_distribution_day(seeded_ctx, pool, membership.household_id)

    result = coord.issue_final_offer(ctx=seeded_ctx, pool_id=pool.id)

    assert not result.issued
    assert result.reason == "no buyer's rules accept the final price"
    assert seeded_ctx.repo.get_pool(WS, pool.id).status == PoolStatus.FAILED


@pytest.mark.parametrize("passes", [2, 3, 4])
def test_a_single_rejection_converges_well_inside_the_bound(seeded_ctx, monkeypatch, passes):
    ...
    assert len(result.economics["lines"]) == len(_active(seeded_ctx, pool.id)) == 9
