...
| Public demo URL, reachable with no AWS account | **Verified, but not from a filtered network** | <https://5hhaadit5pdarllqmbj24u4ybm0ixsyj.lambda-url.us-east-1.on.aws/verify> — the final-audit release, deployed and driven end to end 2026-08-23. Recheck from a private window before submitting. **`*.lambda-url.*.on.aws` is a blocked category on Cisco Umbrella and its peers**, so a judge on a filtered university or corporate network gets a certificate error, not Pool (#0065). The CloudFront distribution that fixes this is implemented and **not yet deployed** |
