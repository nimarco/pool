...
| Public judge demo | ✅ verified | `make deploy-demo` — one Lambda, one Function URL, one table. A CloudFront distribution in front of the Function URL is **deployed and verified 2026-09-02** (#0065), because `*.lambda-url.*.on.aws` is blocked as a category by filtered resolvers; it is the canonical judge URL. |
